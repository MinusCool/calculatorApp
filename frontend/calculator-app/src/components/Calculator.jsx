...
const Calculator = () => {
  ...
  const handleClick = (value) => {
    ...
    if (value === '=') {
      ...
      if (!operator) {
        alert("Invalid format. Please include one operator (+, -, *, /).")
        return
      }
      ...
      axios.post('http://localhost:8000/calculate', {
        ...
        .catch(err => {
          ...
          alert(detail || "An error occurred during calculation.")
        })
      return
    }
    ...
    <div className="calculator">
      ...
      <div className="buttons">
        {buttons.map((btn, index) => (
          btn.trim() === '' ? (
            <button key={index} className="empty" disabled aria-hidden="true" />
          ) : (
